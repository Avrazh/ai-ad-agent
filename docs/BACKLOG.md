...
| F11 | Headline reuse & variation logic | Track headline performance (times shown, approved, rejected) and use it to balance reuse and variation during ad generation. Reuse high-performing headlines, reduce frequency if overused, avoid low-performing ones, and generate new variations when needed. Goal: keep ads fresh while maintaining proven, high-quality messaging. | open | — |
