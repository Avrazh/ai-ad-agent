...
- serif + split_right + geometric_block + right textAlign + outlined label = fashion magazine`,
    ...
    const parsed = JSON.parse(text) as SurpriseSpec;

    // Validate required fields
    const validLayouts = ["top_bottom", "split_left", "split_right", "full_overlay", "bottom_bar", "color_block", "frame_overlay", "magazine"];
    ...
    console.log(`[surprise] layout=${parsed.layout} font=${parsed.font}/${parsed.fontWeight} ls=${parsed.letterSpacingKey}`);
    console.log(`[surprise] bg=${parsed.bgColor} text=${parsed.textColor} accent=${parsed.accentColor} imgOpacity=${parsed.imageOpacity ?? 1}`);
    console.log(`[surprise] label="${parsed.label ?? "-"}" callout="${parsed.calloutText ?? "-"}" decoration=${parsed.decoration ?? "none"}`);
    console.log(`[surprise] en: "${parsed.en?.headline}" / "${parsed.en?.subtext}"`);
